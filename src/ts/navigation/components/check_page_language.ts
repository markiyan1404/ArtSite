...
export const contentUpdate = (language: string): void => {
    ...
    if (isTemplate) {
        ...
        import(`origin/content/${pageName}/${pageName}.json`)
            ...
            .catch(() => import(`origin/content/${pageName}/content_files/${pageName}.json`).then(content => importContent(content["default"], language)))
            .catch(() => import(`origin/content/${pageType}/${pageName}.json`).then(content => importContent(content["default"], language)));
    }
};
